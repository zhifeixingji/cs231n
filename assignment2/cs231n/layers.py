...
def batchnorm_backward(dout, cache):
    ...
    dx = 1 / np.sqrt(var + eps) * dx_hat + dvar * 2.0 / N * (x - mean) + 1.0 / N * dmean
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            for f in range(F):
                for n in range(N):
                    x_pad_masked = x_pad[n, :, i * stride:i * stride + HH, j * stride:j * stride + WW]
                    db[f] += dout[n, f, i, j]
                    dw[f, :, :, :] += x_pad_masked * dout[n, f, i, j]
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            for n in range(N):
                for c in range(C):
                    x_masked = x[n, c, i * stride: i * stride + HH, j * stride: j * stride + WW]
                    dx[n, c, i * stride: i * stride + HH, j * stride: j * stride + WW] += (x_masked == np.max(x_masked)) * dout[n, c, i, j]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
