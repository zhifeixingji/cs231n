...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(hidden_dims)):
            j = hidden_dims[i]
            self.params['W%d' % (i + 1)] = weight_scale * np.random.randn(layer_input_dim, j)
            self.params['b%d' % (i + 1)] = weight_scale * np.zeros(j)
            if self.use_batchnorm:
                self.params['gamma%d' % (i + 1)] = np.ones(j)
                self.params['beta%d' % (i + 1)] = np.zeros(j)
            layer_input_dim = j
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            if self.use_batchnorm:
                layer_input, cache[i] = affine_bn_relu_forward(layer_input,
                                                           self.params['W%d' % (i + 1)],
                                                           self.params['b%d' % (i + 1)],
                ...
                layer_input, cache[i] = affine_relu_forward(layer_input, self.params['W%d' % (i + 1)],
                                                        self.params['b%d' % (i + 1)])

            if self.use_dropout:
                layer_input, dp_cache[i] = dropout_forward(layer_input, self.dropout_param)

        scores, cache[self.num_layers] = affine_forward(layer_input, self.params['W%d' % self.num_layers],
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.use_batchnorm:
                dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dout, cache[lay])
            else:
                dx, dw, db = affine_relu_backward(dout, cache[lay])
